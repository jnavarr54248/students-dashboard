...
def actualizar_graficos(prep, gender, eth):
    ...
    fig1 = px.scatter(filtro, x="math_score", y="reading_score", color="parental_level_of_education", size="writing_score", hover_data=['lunch'], title="📐 Matemáticas vs Lectura", template="plotly_white", color_discrete_sequence=px.colors.qualitative.Prism)

    fig2 = px.box(filtro, x="parental_level_of_education", y="writing_score", color="parental_level_of_education", title="✍️ Escritura por Nivel Educativo de Padres", template="plotly_white", color_discrete_sequence=px.colors.qualitative.Safe)

    barras = df[(df['gender'] == gender) & (df['test_preparation_course'] == prep)]
    fig3 = px.bar(barras.groupby('race/ethnicity')[['math_score', 'reading_score', 'writing_score']].mean().reset_index(), x='race/ethnicity', y=['math_score', 'reading_score', 'writing_score'], barmode='group', title="📊 Promedio de Puntajes por Grupo Socioeconómico", template="plotly_white", labels={"value": "Promedio Puntaje", "race/ethnicity": "Grupo"}, color_discrete_sequence=px.colors.qualitative.Bold)

    fig4 = px.histogram(filtro, x="math_score", nbins=20, color="parental_level_of_education", title="📊 Distribución de Puntajes en Matemáticas", template="plotly_white", color_discrete_sequence=px.colors.qualitative.Pastel)

    fig5 = px.imshow(df[["math_score", "reading_score", "writing_score"]].corr(), text_auto=True, color_continuous_scale='Viridis', title="🔍 Correlación entre Puntajes Académicos", template="plotly_white")
    ...
    fig6.update_layout(polar=dict(radialaxis=dict(visible=True)), showlegend=False, title="📈 Comparación Radar de Puntajes")

    pie_df = filtro['parental_level_of_education'].value_counts().reset_index()
    pie_df.columns = ['Nivel educativo', 'Cantidad']
    fig7 = px.pie(pie_df, values='Cantidad', names='Nivel educativo', title="🧠 Nivel Educativo de Padres", color_discrete_sequence=px.colors.qualitative.Set3)

    fig8 = px.violin(df[df['test_preparation_course'] == prep], x="gender", y="reading_score", color="gender", box=True, points="all", title="📚 Distribución de Lectura por Género", template="plotly_white")
    ...
    fig9 = px.line(linea_df, x="parental_level_of_education", y=["math_score", "reading_score", "writing_score"], markers=True, title="📉 Tendencias por Educación Parental", labels={"value": "Promedio", "variable": "Materia"}, template="plotly_white")
